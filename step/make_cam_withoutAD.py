...
def _work(process_id, model, dataset, args):
    ...
    with cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            directory, file_name = os.path.split(img_name) #file_name = benign (1).png
            subdirectories = directory.split('/') #['', 'home', 'lintzuh@kean.edu', 'BUS', 'data', 'Dataset_BUSI_with_GT', 'benign']

            os.makedirs(os.path.join(args.cam_out_dir, subdirectories[-1]), exist_ok=True)
            # cam_out's default = 'result/cams'
            if os.path.exists(os.path.join(args.cam_out_dir, subdirectories[-1],file_name[:-4] + '.npy')):
                continue
            ...
            outputs_cam1 = []
            outputs_cam2 = []
            n_classes = len(list(torch.nonzero(F.one_hot(pack['label'], num_classes=2)[0])[:, 0]))

            # 1
            cam_all_classes = []
            for size_idx in range(len(pack['img'])):
                img_single = pack['img'][size_idx].cuda(non_blocking=True)

                outputs = gcam.forward(img_single, step=1)
                for c in range(n_classes):
                    gcam.backward(ids=c)
                    regions = gcam.generate(target_layer="stage4")
                    regions = regions[0] + regions[1].flip(-1)

                    if size_idx == 0 and c == 0:
                        cam_all_classes = torch.zeros([n_classes, *regions.size()[1:]])

                    cam_all_classes[c] += regions.data.cpu()

            cam_all_classes /= len(pack['img'])  # Average over all sizes

            # Save the CAMs
            np.save(os.path.join(args.cam_out_dir, subdirectories[-1], file_name[:-4] + '.npy'),
                    cam_all_classes.numpy())
            print(f"Processed {file_name}")

        if process_id == n_gpus - 1:
            print(f"{iter+1} images processed on GPU {process_id}")
            
            return 
            # 2
            for s_count, size_idx in enumerate([1, 2, 0, 3, 4, 5, 6]):
                orig_img = pack['img'][size_idx].clone()
                for c_idx, c in enumerate(list(torch.nonzero(F.one_hot(pack['label'], num_classes=2)[0])[:, 0])):
                    ...
                    for it in range(total_adv_iter):
                        ...
                        valid_cat = torch.nonzero(F.one_hot(pack['label'], num_classes=2)[0])[:, 0]
