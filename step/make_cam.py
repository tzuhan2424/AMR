...
def _work(process_id, model, dataset, args):
    ...
    with cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            # Separate into directory and file name
            ...
            outputs_cam1 = []
            outputs_cam2 = []
            # test1 = F.one_hot(pack['label'], num_classes=2)[0]
            # test2 = torch.nonzero(F.one_hot(pack['label'], num_classes=2)[0])
            # test = list(torch.nonzero(F.one_hot(pack['label'], num_classes=2)[0])[:, 0])
