...
cls_labels_dict = np.load('voc12/cls_labels_tumor.npy', allow_pickle=True).item()
...
def get_img_path_tumor(img_name):
    if img_name[0] == 'b':
        type = 'benign'
    elif img_name[0] == 'm':
        type = 'malignant'
        ...
        type = 'normal'
    return os.path.join('../Dataset_BUSI_with_GT/', type , img_name)
...
def load_img_name_list_tumor(dataset_path):

    # img_name_list = np.loadtxt(dataset_path, dtype='str',delimiter='\n')
    with open(dataset_path, "r") as file:
        img_name_list = [line.strip() for line in file]
...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
        ...
        self.img_name_list = load_img_name_list(img_name_list_path) #[2011003256, 2011003271] and it is a number
...
class BreastCancer(Dataset):
    ...
    def __init__(self, img_name_list_path, img_normal=TorchvisionNormalize(),to_torch=True):
        self.img_name_list = load_img_name_list_tumor(img_name_list_path) #['benign (132).png' 'normal (9).png']
        self.img_normal = img_normal
    ...
    def __getitem__(self, idx):
        ...
        img = imageio.imread(get_img_path_tumor(name))
        #resize the image in order to stack the tensor
        target_size = (224, 224)
...
class BreastCancerClassificationMSF(BreastCancer):
    def __init__(self, img_name_list_path,
                 img_normal=TorchvisionNormalize(),
                 scales=(1.0,)):
        self.scales = scales

        super().__init__(img_name_list_path, img_normal=img_normal)
        self.scales = scales
...
class BreastCancerSegmentation(Dataset):
    def __init__(self, img_name_list_path, label_dir,img_normal=TorchvisionNormalize()):
        self.img_name_list = load_img_name_list_tumor(img_name_list_path) #['benign (132).png' 'normal (9).png']
        self.img_normal = img_normal
        self.label_dir = label_dir
